//! Crash handler: persist a readable log when optionfiles-gtk panics.
//!
...
//! message, backtrace) to `~/.option/files/crash.log`, once per process, so
...
/// call chain is huge; the head is the part that names optionfiles symbols.
const MAX_FRAMES: usize = 120;

/// The panic hook is installed once, before any GTK code runs.
/// `option_sdk::App::FILES` gives the `~/.option/files` directory shared by
/// the rest of the app.
pub fn install() {
...
/// `~/.option/files/crash.log`, per optionSDK.
fn crash_log_path() -> PathBuf {
    option_sdk::App::FILES.dir().join("crash.log")
}
...
mod tests {
    ...
    fn crash_log_writes_message() {
        let dir = std::env::temp_dir().join("optionfiles-gtk-crash-test");
